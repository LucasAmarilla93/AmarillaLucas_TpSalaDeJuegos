...
export class Estadisticasjuegos {
  ...
  tablaResultados: PostgrestQueryBuilder <any, any, any, 'resultados', unknown> = this.supabService.supabase.from('resultados');
  ...
  async guardarResultado(puntaje: number, juego: string){   
    ...
    const {data} = await this.supabService.supabase.from('resultados').select('*').eq('usuario', this.usuario);
    const puntajePrevio = data?.[0]?.[juego];
    console.log("Puntaje", [data?.[0]?.[juego]])
